...
fn bench_encode_megabyte(c: &mut Criterion) {
    ...
    c.bench_function("bench_encode_megabyte", move |b| b.iter(|| encode(&*bytes, SpecialChars::default())));
}
...
fn bench_decode_megabyte(c: &mut Criterion) {
    ...
    bytes[0] = 0x7E;
    bytes[999_999] = 0x7E;
    c.bench_function("bench_decode_megabyte", move |b| b.iter(|| decode(&*bytes, SpecialChars::default())));
}

fn bench_encode_special_chars_megabyte(c: &mut Criterion) {
    let bytes = Box::new(vec![0x7E as u8; 1_000_000]);
    c.bench_function("bench_encode_special_chars_megabyte", move |b| b.iter(|| encode(&*bytes, SpecialChars::default())));
}
...
fn bench_decode_special_chars_2_megabytes(c: &mut Criterion) {
    ...
    c.bench_function("bench_decode_special_chars_2_megabytes", move |b| b.iter(|| decode(&*bytes, SpecialChars::default())));
}
...
criterion_group!(benches, bench_encode_megabyte, bench_decode_megabyte, bench_encode_special_chars_megabyte, bench_decode_special_chars_2_megabytes);
